...
const checkFraud = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await LeadServices.checkFraudByPhone(phone as string);
...
const getSingleLead = catchAsync(
...
const updateLead = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const lead = await LeadServices.updateLeadService(
      payload,
      leadId,
      decodedToken,
    );
...
const getAllTrashLeads = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const query = req.query;
    const result = await LeadServices.getAllTrashLeads(
      query as Record<string, string>,
    );
...
const updateLeadTrash = catchAsync(async (req: Request, res: Response) => {
  const id = req.params.id as string;
